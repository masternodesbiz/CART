...
extern UniValue DoZcartSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzcartseed(const UniValue& params, bool fHelp);
extern UniValue getzcartseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzcart(const UniValue& params, bool fHelp);
extern UniValue dzcartstate(const UniValue& params, bool fHelp);
